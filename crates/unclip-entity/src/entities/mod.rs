//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.20

pub mod prelude;

pub mod alignment_candidates;
pub mod alignments;

pub mod branch_o2m_values;
pub mod branch_o2o_values;
pub mod branch_references;
pub mod branches;
pub mod candidate_interpretations;
pub mod candidates;
pub mod domain_revision_interpretations;
pub mod domain_revisions;
pub mod domain_versions;
pub mod domains;
pub mod empirical_structures;
pub mod engine_runs;
pub mod experiment_deltas;
pub mod experiment_observations;
pub mod experiments;
pub mod frame_axes;
pub mod frame_slot_o2m_values;
pub mod frame_slot_o2o_values;
pub mod frame_slots;
pub mod frame_versions;
pub mod frames;
pub mod measurement_frames;
pub mod measurement_profiles;
pub mod measurements;
pub mod observations;
pub mod observed_relations;
pub mod observed_units;
pub mod pattern_entries;
pub mod provenance;
pub mod provenance_inputs;
pub mod ranking_entries;
pub mod rankings;
pub mod relation_properties;
pub mod relations;
pub mod selection_packets;
pub mod sensor_runs;
pub mod unit_properties;
pub mod units;
pub mod usage_history;
