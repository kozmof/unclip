...
impl CandidateGenerator for MissingRelationGenerator {
    ...
    fn generate(
        ...
        let residuals = residual_evidence(ctx, "unexplained_relations")?;
        let mut groups = BTreeMap::<_, Vec<serde_json::Value>>::new();
        let mut support = BTreeMap::<_, BTreeSet<_>>::new();
        for (id, measurements) in residuals {
            let (observation, relation, key) = relations.get(&id).ok_or_else(|| {
                invalid("residual relation is not present in selected observations")
            })?;
            if key.0.trim().is_empty() || key.1.trim().is_empty() || key.2.trim().is_empty() {
                continue;
            }
            if relation
                .uncertainty
