//! Integration tests for the store repositories over in-memory SQLite.
//!
//! These live in `tests/` (not in `src/lib.rs`) so they exercise the crate's
...
/// A one-element packet record for tests that persist a single packet.
fn packet_record(id: &str, seed: Option<u64>, branch_ids: Vec<i64>) -> PacketUsageRecord {
    PacketUsageRecord {
        id: id.to_string(),
        frame_name: None,
        seed,
